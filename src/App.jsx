...
import { Search, Sprout, TreeDeciduous, Info, CheckCircle2, XCircle, Calculator, BoxSelect, HelpCircle, Zap } from 'lucide-react';
...
export default function App() {
    ...
    const renderIcon = (item, sizeClass = "w-16 h-16") => {
        if (item.image && !imgError) {
            return (
                <img
                    src={item.image}
                    alt={item.name}
                    className={`${sizeClass} object-contain rounded-md bg-black/20`}
                    onError={() => setImgError(true)}
                />
            );
        }
        return <div className={`${sizeClass} rounded-full ${item.color} shadow-inner border border-white/10`}></div>;
    };
                ...
                <aside className="lg:col-span-4 space-y-6">
                    ...
                    <div className="bg-slate-800 rounded-xl p-4 border border-slate-700 shadow-lg">
                        ...
                        <p className="text-xs text-slate-500 mt-2 text-center">
                            Hover over cells to see full names.
                        </p>
                    </div>
                </aside>
                ...
                <section className="lg:col-span-8 space-y-6">
                    {selectedMutation ? (
                        <>
                            ...
                            <div className="bg-slate-800 border-l-4 border-blue-500 rounded-xl p-6 shadow-lg">
                                <div className="flex flex-col sm:flex-row gap-6">
                                    {/* Image */}
                                    <div className="flex-shrink-0 flex items-center justify-center bg-slate-900 rounded-lg p-2 w-20 h-20 sm:w-24 sm:h-24 border border-slate-700">
                                        {renderIcon(selectedMutation, "w-full h-full")}
                                    </div>
                            ...
                            {/* INFO BOXES: HOW TO GET & PERKS */}
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">

                                {/* How To Get */}
                                <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-lg flex flex-col h-full">
                                    <h3 className="text-sm font-semibold text-blue-400 mb-4 uppercase tracking-wider flex items-center gap-2 border-b border-slate-700 pb-2">
                                        <HelpCircle className="w-4 h-4" /> How to Obtain
                                    </h3>
                                    {/* whitespace-pre-line is crucial for handling \n in strings */}
                                    <div className="text-sm text-slate-300 leading-relaxed whitespace-pre-line flex-grow">
                                        {selectedMutation.howToGet ? (
                                            selectedMutation.howToGet
                                        ) : (
                                            <span className="text-slate-500 italic">No specific harvest instructions provided.</span>
                                        )}
                                    </div>
                                </div>
                                ...
                                {/* Perks */}
                                <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-lg flex flex-col h-full">
                                    <h3 className="text-sm font-semibold text-yellow-500 mb-4 uppercase tracking-wider flex items-center gap-2 border-b border-slate-700 pb-2">
                                        <Zap className="w-4 h-4" /> Perks
                                    </h3>

                                    <div className="flex-grow">
                                        {selectedMutation.perks && selectedMutation.perks.length > 0 ? (
                                            <div className="space-y-4">
                                                {selectedMutation.perks.map((perk, idx) => (
                                                    <div key={idx} className="bg-slate-900/40 p-3 rounded border border-slate-700/50">
                                                        {typeof perk === 'object' ? (
                                                            <>
                                                                <div className="font-semibold text-slate-200 text-sm mb-1">{perk.name}</div>
                                                                <div className="text-slate-400 text-xs leading-relaxed">{perk.desc || perk.description}</div>
                                                            </>
                                                        ) : (
                                                            <span className="text-slate-300 text-sm whitespace-pre-line">{perk}</span>
                                                        )}
                                                    </div>
                                                ))}
                                            </div>
                                        ) : (
                                            <p className="text-sm text-slate-500 italic">No special perks listed.</p>
                                        )}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-slate-500 border-2 border-dashed border-slate-700 rounded-xl min-h-[400px]">
                            ...
                            <p className="text-lg font-medium">Select a mutation to begin</p>
                        </div>
                    )}
                </section>
            </main>
